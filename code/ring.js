...
export class Ring {
    constructor(gl, innerRadius = 5.0, outerRadius = 7.0, segments = 32, color = [0.8, 0.8, 0.8]) {
        this.gl = gl;
    ...
    loadTextures() {
        ...
        const ringTexture = new Image();
        ringTexture.onload = () => {
            gl.bindTexture(gl.TEXTURE_2D, this.texture);
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, ringTexture);
            gl.generateMipmap(gl.TEXTURE_2D);
        };
        ringTexture.src = 'textures/ring.jpg';
        

        const ringNormalMap = new Image();
        ringNormalMap.onload = () => {
            gl.bindTexture(gl.TEXTURE_2D, this.normalMap);
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, ringNormalMap);
            gl.generateMipmap(gl.TEXTURE_2D);
        };
        ringNormalMap.src = 'textures/ring_norm.png';
    }
    ...
    initBuffers() {
        ...
        for (let i = 0; i <= this.segments; i++) {
            ...
            vertices.push(
                this.innerRadius * cosTheta,
                0,
                this.innerRadius * sinTheta
            );
            normals.push(0, 1, 0);
            texCoords.push(i / this.segments, 0);


            vertices.push(
                this.outerRadius * cosTheta,
                0,
                this.outerRadius * sinTheta
            );
            normals.push(0, 1, 0);
            texCoords.push(i / this.segments, 1);
        }
    ...
    draw(viewProj, cameraPos, lights) {
        ...
        gl.uniform1i(this.uUseNormalMap, 1); // 노말맵 사용 활성화
